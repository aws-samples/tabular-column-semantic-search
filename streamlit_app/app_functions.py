...
def lambda_query_opensearch_and_reduce_embeddings(domain, index, search_vector, k_query, payload_embedding, lambda_arn):
    ...
    if lambda_return_dict['index_exists']==True:
...
def display_visualization(result_dict, k_user_input, payload_reduced):
    ...
    fig = px.scatter(result_df, x='x_reduced', y='y_reduced', 
                    hover_data = hover_labels,
                    labels=dict(x_reduced="x", y_reduced="y"))
    ...
    for i in range(k_user_input):
        ...
        fig.add_trace(
            go.Scatter( x=[payload_reduced[0], x_nn], y=[payload_reduced[1], y_nn], mode = 'lines+markers' , name = column_name)
        )
    # Plot payload
    fig.add_trace(
        go.Scatter(x = [payload_reduced[0]], y = [payload_reduced[1]], name = "Payload", legendrank=1)
    )
    # Display
    fig.update_layout(title_text='EDA Visualization*', title_x=0.5)
    # fig.update_xaxes(visible=False)
    st.plotly_chart(fig)
    st.markdown("<p style='text-align: left; color: grey;'>*2D visualization may skew perceived nearest neighbors due to dimensionality reduction.</p>", unsafe_allow_html=True)