...
def handler(event, context):
    ...
    try:
        ...
        res = s3_move_objects(
            ...
            source_prefix=input_data_path, 
            dest_bucket=bucket, 
            dest_prefix=processed_csv_s3_path)

        if res == None:
            result_dict['csv'] = {
                    'source': input_data_path,
                    'destination': processed_csv_s3_path
                }
        else: 
            result_dict['csv'] = res
        ...
        res = s3_move_objects(
            source_bucket=bucket, 
            source_prefix=parquet_source_prefix, 
            dest_bucket=bucket, 
            dest_prefix=parquet_s3_path)

        if res == None:
            result_dict['parquet'] = {
                    'source': parquet_source_prefix,
                    'destination': parquet_s3_path
                }
        else: 
        ...
        if res == None:
            result_dict['embeddings'] = {
                    'source': embeddings_source_prefix,
                    'destination': embeddings_s3_path
                }
        else: 
