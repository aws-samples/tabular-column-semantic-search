...
def handler(event, context):
    ...
    for key in key_list:
        ...
        index = key.replace(f"{processing_prefix}/", "").replace(f"/{table_name}.h5", "").replace("/", "-").replace("_", "-").lower()

        if index not in result_dict:
            result_dict[index] = {}

        result_dict[index][table_name] = index_h5_embeddings_to_opensearch(
