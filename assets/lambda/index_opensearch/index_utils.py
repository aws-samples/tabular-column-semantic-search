...
def get_h5_embedding_data(file):
    """Get embeddings data from h5 file."""
...
def create_index(connection, index, len_embeddings):
    ...
    knn_index = {
        "settings": {"index": {"knn": True, "knn.algo_param.ef_search": 256, "knn.space_type": "cosinesimil"}},
        "mappings": {
            "properties": {
    ...
    if connection.indices.exists(index=index):
        ...
        res = connection.indices.create(index=index, body=knn_index, ignore=400)
        logger.info(res)
