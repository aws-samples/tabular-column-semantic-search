...
def query_opensearch(connection, index, search_vector, k, payload):
    ...
    res = connection.search(request_timeout=60, 
                        index=index,
                        body=query)
...
def reduce_embedding_dimensions(result_dict, search_vector, payload_embedding):
    ...
    tsne_embeddings_arr = TSNE(n_components=2, verbose=0, init='pca', metric='cosine', perplexity=30, random_state=123).fit_transform(search_vector_embeddings_arr_temp) 
